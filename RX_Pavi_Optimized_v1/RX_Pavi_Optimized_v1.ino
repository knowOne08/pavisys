...
void showHelpMenu();
void waitForAck(String originalCommand);
void processAckResponse(String ackMessage, String originalCommand); 
...
void setup() {
  ...
  // ESP32-C3 Fix: Ultra-conservative LoRa settings (matching TX)
  LoRa.setSpreadingFactor(12);    // Maximum reliability
  LoRa.setSignalBandwidth(125E3); // Standard bandwidth (125kHz) - matches TX
  LoRa.setCodingRate4(8);         // Maximum error correction
  LoRa.enableCrc();               // Enable CRC
  LoRa.setSyncWord(0x12);         // Custom sync word
  
  // ESP32-C3: Additional stability settings (matching TX)
  LoRa.setPreambleLength(12);     // Match TX: 12 symbols instead of 8
  LoRa.setTxPower(6, PA_OUTPUT_PA_BOOST_PIN); // Match TX: 20dBm maximum power
  
  delay(500); // Long LoRa settling time
  
  Serial.println("LoRa initialized with ultra-conservative settings");
  Serial.println("TX Power: 20dBm (MAXIMUM - matching TX settings)");
  Serial.println("Spreading Factor: 12 (Maximum range - matching TX)");
  Serial.println("Preamble Length: 12 symbols (matching TX)");
...
void handleLoRaReceive() {
  ...
  if (packetSize) {
    ...
    // Check if this is an ACK message (handled separately in waitForAck)
    if (!received.startsWith("ACK:")) {
      // Process non-ACK data normally
...
void processReceivedData(String data) {
  // Handle different types of responses from TX (excluding ACKs)
...
void sendLoRaCommand(String command) {
  ...
  if (ESP32_C3_MODE) {
    ...
    LoRa.endPacket();
    yield();
    // NO delay here - start listening immediately
  } else {
    LoRa.beginPacket();
    LoRa.print(command);
    LoRa.endPacket();
  ...
  // Resume receiving immediately - no delays
  LoRa.receive();
  
  // Wait for ACK response with minimal delay
  waitForAck(command);
}

// Wait for ACK response from TX
void waitForAck(String originalCommand) {
  // Show waiting message immediately since TX may take time to process
  Serial.println("⏳ Waiting for TX acknowledgment...");
  
  // Start timing immediately
  unsigned long startTime = millis();
  unsigned long timeout = 5000; // Increased to 5 second timeout for complex commands
  ...
  while (millis() - startTime < timeout && !ackReceived) {
    ...
    if (packetSize > 0) {
      ...
      // Check if this is an ACK for our command
      if (received.startsWith("ACK:")) {
        ackReceived = true;
        processAckResponse(received, originalCommand);
        return; // Exit immediately on ACK received
      } else {
        // Process non-ACK data normally
        processReceivedData(received);
      }
    ...
    // Ultra-fast polling - 1ms delay for maximum responsiveness
    delay(1);
  }
  
  if (!ackReceived) {
    Serial.println("⚠️ No ACK received from TX (timeout after " + String(timeout/1000) + "s)");
    Serial.println("💡 TX may be out of range or experiencing issues");
  }
  
  // Resume normal LoRa receiving
  LoRa.receive();
}

// Process ACK responses from TX
void processAckResponse(String ackMessage, String originalCommand) {
  // Parse ACK format: ACK:COMMAND:STATUS:MESSAGE
  int colon1 = ackMessage.indexOf(':', 0);
  int colon2 = ackMessage.indexOf(':', colon1 + 1);
  int colon3 = ackMessage.indexOf(':', colon2 + 1);
  
  if (colon1 > 0 && colon2 > colon1 && colon3 > colon2) {
    String ackCommand = ackMessage.substring(colon1 + 1, colon2);
    String status = ackMessage.substring(colon2 + 1, colon3);
    String message = ackMessage.substring(colon3 + 1);
    
    if (status == "OK") {
      Serial.println("✅ TX ACK: " + ackCommand + " - " + message);
    } else if (status == "ERROR") {
      Serial.println("❌ TX ERROR: " + ackCommand + " - " + message);
    } else {
      Serial.println("⚠️ TX UNKNOWN STATUS: " + ackCommand + " - " + status + " - " + message);
    }
  } else {
    // Fallback for malformed ACK
    Serial.println("📨 TX Response: " + ackMessage);
  }
}
